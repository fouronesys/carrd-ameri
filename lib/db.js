...
// Cuenta cuántas veces se ha usado un código promocional. Se cuentan todos los
// agendamientos que lo llevan aplicado y que no fueron rechazados (así los cupos
// se liberan solos si un pago se rechaza o si se limpia una reserva vencida).
function contarUsosCodigo(codigoStr) {
  const cod = String(codigoStr || '').trim().toLowerCase();
  if (!cod) return 0;
  const regs = listarAgendamientos();
  let n = 0;
  for (let i = 0; i < regs.length; i++) {
    if (regs[i].estado === 'rechazado') continue;
    if (String(regs[i].codigo_promo || '').trim().toLowerCase() === cod) n++;
  }
  return n;
}
