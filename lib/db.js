...
async function crearEsquema() {
  await q(`CREATE TABLE IF NOT EXISTS agendamientos (
    ref  TEXT PRIMARY KEY,
    seq  BIGSERIAL,
    data JSONB NOT NULL
  )`);
  await q(`CREATE INDEX IF NOT EXISTS agendamientos_pedido_idx ON agendamientos ((data->>'pedido_ref'))`);
  await q(`CREATE INDEX IF NOT EXISTS agendamientos_creado_idx ON agendamientos ((data->>'creado_en'))`);
  await q(`CREATE INDEX IF NOT EXISTS agendamientos_codigo_idx ON agendamientos ((lower(data->>'codigo_promo')))`);
  ...
  await q(`CREATE TABLE IF NOT EXISTS codigos (
    id   TEXT PRIMARY KEY,
    seq  BIGSERIAL,
    data JSONB NOT NULL
  )`);
  ...
  await q(`CREATE TABLE IF NOT EXISTS usuarios_admin (
    id      TEXT PRIMARY KEY,
    usuario TEXT UNIQUE,
    data    JSONB NOT NULL
  )`);
  await q(`CREATE TABLE IF NOT EXISTS accesos (
  ...
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS accesos_id_idx ON accesos ((data->>'id'))`);
...
async function migrarDesdeJSON(opciones) {
  ...
  if (await toca('agendamientos')) {
    const datos = leerJSON(DB_FILE, { agendamientos: [] });
    const lista = Array.isArray(datos.agendamientos) ? datos.agendamientos : [];
    let n = 0;
    ...
    await marcarMigracion('agendamientos');
  ...
  if (await toca('promos')) {
    const lista = Array.isArray(promoData.promos) ? promoData.promos : [];
    let n = 0;
    ...
    await marcarMigracion('promos');
    resumen.promos = n;
  }
  if (await toca('codigos')) {
    const lista = Array.isArray(promoData.codigos) ? promoData.codigos : [];
    let n = 0;
    ...
    await marcarMigracion('codigos');
    resumen.codigos = n;
  }

  // Clientes
  if (await toca('clientes')) {
    const datos = leerJSON(CLIENTES_FILE, { clientes: [] });
    const lista = Array.isArray(datos.clientes) ? datos.clientes : [];
    let n = 0;
    for (const c of lista) {
      if (!c || !c.id) continue;
      const r = await q(
    ...
    await marcarMigracion('clientes');
    resumen.clientes = n;
  }

  // Usuarios del panel
  const usuData = leerJSON(USUARIOS_FILE, { usuarios: [], accesos: [] });
  if (await toca('usuarios_admin')) {
    const lista = Array.isArray(usuData.usuarios) ? usuData.usuarios : [];
    let n = 0;
    ...
    await marcarMigracion('usuarios_admin');
    resumen.usuarios = n;
  }
  ...
  if (await toca('accesos')) {
    const original = Array.isArray(usuData.accesos) ? usuData.accesos : [];
...
async function verificarMigracion() {
  ...
  const detalle = {};
  let ok = true;
  for (const tabla of Object.keys(fuentes)) {
