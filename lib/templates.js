...
const GA_ID = 'G-HRVG87V7HP';

// Construye los datos de una compra para GA4 a partir de la referencia y los
// montos disponibles. Los pagos por Wompi se registran en COP; los pagos
// manuales en dólares (precio_cop = 0) se registran en USD.
function datosCompra(ref, montoCop, montoUsd) {
  if (!ref) return null;
  const cop = parseInt(montoCop, 10) || 0;
  if (cop > 0) return { ref: ref, valor: cop, moneda: 'COP' };
  const usd = parseFloat(String(montoUsd || '').replace(',', '.'));
  if (usd > 0) return { ref: ref, valor: Math.round(usd * 100) / 100, moneda: 'USD' };
  return null;
}

// Fragmento de Google Analytics 4. Si se pasa una compra ({ref, valor, moneda}),
// además del pageview se envía un evento "purchase" (una sola vez por sesión
// gracias a una marca en sessionStorage, para no duplicar al recargar).
function gtagSnippet(compra) {
  let extra = '';
  if (compra && compra.ref && Number(compra.valor) > 0) {
    const claveJs = JSON.stringify('fresa_ga_' + String(compra.ref));
    extra = '(function(){var y=false;try{y=!!sessionStorage.getItem(' + claveJs + ');}catch(e){}' +
      'if(!y){gtag("event","purchase",{transaction_id:' + JSON.stringify(String(compra.ref)) +
      ',value:' + Number(compra.valor) + ',currency:' + JSON.stringify(compra.moneda || 'COP') + '});' +
      'try{sessionStorage.setItem(' + claveJs + ',"1");}catch(e){}}})();';
  }
  return '<script async src="https://www.googletagmanager.com/gtag/js?id=' + GA_ID + '"></script>' +
    '<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}' +
    'gtag("js",new Date());gtag("config","' + GA_ID + '");' + extra + '</script>';
}
...
function paginaGracias(opciones) {
  ...
  const detalle = reg.producto
    ? ['<div class="detalle">',
        ...
        reg.adelanto ? '<div class="fila"><span>Adelanto</span><strong>Urgencia · entrega en máx. 3 días</strong></div>' : '',
  ...
  return [
    ...
    gtagSnippet((estado === 'agendado' || estado === 'comprobante_recibido')
      ? datosCompra(reg.ref, reg.precio_cop, reg.precio_usd)
      : null),
...
function paginaGraciasPedido(opciones) {
  ...
  return [
    ...
    '<meta name="viewport" content="width=device-width, initial-scale=1">',
    gtagSnippet((estado === 'agendado' || estado === 'comprobante_recibido')
      ? datosCompra(pedidoRef, total, registros.reduce(function (s, r) {
          const u = parseFloat(String(r.precio_usd || '').replace(',', '.'));
...
function scriptAdmin() {
  return [
    ...
    '      var okTrabajo=filtroActivo!=="realizado"||tr.getAttribute("data-trabajo-hecho")==="true";',
    '      var okBusca=!q||(tr.getAttribute("data-buscar")||"").indexOf(q)!==-1;',
    '      var mostrar=okEstado&&okTrabajo&&okBusca;',
    ...
    '  var secs=document.querySelectorAll(".admin-sec");',
    '  tabs.forEach(function(t){',
    '    t.addEventListener("click",function(){',
    '      var id=t.getAttribute("data-tab");',
    '      tabs.forEach(function(x){x.classList.remove("activo");});',
    '      t.classList.add("activo");',
    '      secs.forEach(function(s){s.classList.toggle("activo",s.getAttribute("data-sec")===id);});',
    '      window.scrollTo(0,0);',
    '    });',
    '  });',
    '})();',
...
/* ---------- Sección de eliminados (papelera) ---------- */
function seccionEliminados(opciones) {
  const eliminados = (opciones && opciones.eliminados) || [];
  const baseUrl = (opciones && opciones.baseUrl) || '';

  if (eliminados.length === 0) {
    return '<div class="admin-sec" data-sec="eliminados">' +
      '<div class="tabla-wrap"><div class="vacia">No hay registros en la papelera.</div></div></div>';
  }
  ...
  const filas = eliminados.map(function (r) {
    ...
    const botonRestaurar = '<form method="POST" action="/admin/restaurar/' + refEnc + '" style="margin:0">' +
      '<button class="btn-acc btn-aprobar" type="submit" onclick="return confirm(\'¿Restaurar este agendamiento?\');">↺ Restaurar</button></form>';
    const botonEliminarDef = '<form method="POST" action="/admin/eliminar-definitivo/' + refEnc + '" style="margin:0">' +
      '<button class="btn-acc btn-del" type="submit" onclick="return confirm(\'¿Eliminar definitivamente? Esta acción NO se puede deshacer.\');">✕ Borrar definitivo</button></form>';
    ...
    return [
      ...
      '<td>' + foto + '</td>',
      '<td>' + comprobante + '</td>',
      '<td class="ev-cell"><span class="vacio">—</span></td>',
      '<td class="info">' + (r.info_extra ? escaparHtml(r.info_extra) : '<span class="vacio">—</span>') + '</td>',
      '<td>' + estadoCol + '</td>',
      '<td class="fecha">' + escaparHtml(formatearFecha(r.eliminado_en || r.creado_en)) + '</td>',
      '<td><div class="acc">' + botonRestaurar + botonEliminarDef + '</div></td>',
      '</tr>',
    ].join('');
  }).join('');

  return '<div class="admin-sec" data-sec="eliminados">' +
    '<h2 style="font-size:18px;font-weight:900;color:' + COLORES.rosa + ';margin:0 0 16px;">✦ Papelera de eliminados</h2>' +
    '<div class="tabla-wrap"><table><thead><tr>' +
    '<th>Servicio</th><th>Valor</th><th>Cliente</th><th>Contacto</th><th>Persona a trabajar</th><th>Foto</th><th>Comprobante</th><th>Evidencia / Cliente</th><th>Información extra</th><th>Estado</th><th>Fecha eliminación</th><th>Acciones</th>' +
    '</tr></thead><tbody>' + filas + '</tbody></table></div></div>';
}

/* ---------- Sección de trabajos realizados ---------- */
function seccionRealizados(opciones) {
  const realizados = (opciones && opciones.realizados) || [];
  const baseUrl = (opciones && opciones.baseUrl) || '';

  if (realizados.length === 0) {
    return '<div class="tabla-wrap"><div class="vacia">No hay trabajos realizados.</div></div>';
  }
  ...
  const filas = realizados.map(function (r) {
    const foto = miniatura(r.objetivo_foto, 'Foto de la persona');
    const comprobante = miniatura(r.comprobante, 'Comprobante de pago');
    const datos = [
      r.objetivo_nombre ? '<div><b>Nombre:</b> ' + escaparHtml(r.objetivo_nombre) + '</div>' : '',
    ...
    const estadoCol = badge(r.estado) +
      (r.metodo === 'transferencia' ? '<span class="badge metodo">Transferencia</span>' : '') +
      '<span class="badge trab">✓ Trabajo realizado</span>';

    const refEnc = encodeURIComponent(r.ref || '');
    const botonDesmarcar = '<form method="POST" action="/admin/trabajo/' + refEnc + '" style="margin:0">' +
      '<input type="hidden" name="accion" value="desmarcar">' +
      '<button class="btn-acc btn-rechazar" type="submit" onclick="return confirm(\'¿Desmarcar como realizado?\');">↺ Desmarcar</button></form>';
    ...
    return [
      ...
      '<td class="info">' + (r.info_extra ? escaparHtml(r.info_extra) : '<span class="vacio">—</span>') + '</td>',
      '<td>' + estadoCol + '</td>',
      '<td class="fecha">' + escaparHtml(formatearFecha(r.trabajo_hecho_en || r.creado_en)) + '</td>',
      '<td><div class="acc">' + botonDesmarcar + '</div></td>',
      '</tr>',
    ].join('');
  }).join('');

  return '<h2 style="font-size:18px;font-weight:900;color:' + COLORES.rosa + ';margin:0 0 16px;">✦ Trabajos realizados</h2>' +
    '<div class="tabla-wrap"><table><thead><tr>' +
    '<th>Servicio</th><th>Valor</th><th>Cliente</th><th>Contacto</th><th>Persona a trabajar</th><th>Foto</th><th>Comprobante</th><th>Evidencia / Cliente</th><th>Información extra</th><th>Estado</th><th>Fecha realizado</th><th>Acciones</th>' +
...
function adminDashboard(opciones) {
  ...
  // Incluir eliminados y realizados con pago confirmado en el cálculo de ingresos
  const eliminadosConPago = eliminados.filter(function (r) { return r.estado === 'agendado'; });
  const realizadosConPago = realizados.filter(function (r) { return r.estado === 'agendado'; });
  const ingresosCOP = agendadosList.reduce(function (sum, r) { return sum + (parseInt(r.precio_cop, 10) || 0); }, 0) +
    eliminadosConPago.reduce(function (sum, r) { return sum + (parseInt(r.precio_cop, 10) || 0); }, 0) +
    realizadosConPago.reduce(function (sum, r) { return sum + (parseInt(r.precio_cop, 10) || 0); }, 0);
  ...
  const filas = registros.map(function (r) {
    ...
    const botonTrabajo = '<form method="POST" action="/admin/trabajo/' + refEnc + '" style="margin:0">' +
      '<input type="hidden" name="accion" value="' + (r.trabajo_hecho ? 'desmarcar' : 'marcar') + '">' +
    ...
    return [
      '<tr data-estado="' + escaparHtml(r.estado || '') + '" data-trabajo-hecho="' + escaparHtml(r.trabajo_hecho ? 'true' : 'false') + '" data-buscar="' + escaparHtml(buscable) + '">',
  ...
  return [
    ...
    '.admin-nav{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:22px;border-bottom:1px solid rgba(194,167,183,0.22);}',
    '.admin-tab{font-size:12.5px;font-weight:700;padding:10px 18px;border:none;border-bottom:2px solid transparent;background:none;color:' + COLORES.tenue + ';cursor:pointer;border-radius:8px 8px 0 0;letter-spacing:0.03em;font-family:inherit;margin-bottom:-1px;}',
    '.admin-tab:hover{color:' + COLORES.rosa + ';}',
    '.admin-tab.activo{color:' + COLORES.rosa + ';border-bottom-color:' + COLORES.rosa + ';background:rgba(125,55,84,0.14);}',
    '.admin-sec{display:none;}',
    '.admin-sec.activo{display:block;}',
    ...
    '<nav class="admin-nav">' +
      '<button class="admin-tab activo" data-tab="agenda">Agendamientos</button>' +
      '<button class="admin-tab" data-tab="realizados">Realizados</button>' +
      (esAsistente ? '' :
        '<button class="admin-tab" data-tab="promos">Promociones</button>' +
        '<button class="admin-tab" data-tab="usuarios">Usuarios y seguridad</button>' +
        '<button class="admin-tab" data-tab="eliminados">Eliminados</button>') +
    '</nav>',
    '<div class="admin-sec activo" data-sec="agenda">',
    '<div class="stats">',
    ...
    '</div>',
    (esAsistente ? '' : '<div class="admin-sec" data-sec="promos">' + seccionPromociones(opciones) + '</div>'),
    (esAsistente ? '' : '<div class="admin-sec" data-sec="usuarios">' + seccionUsuarios(opciones) + '</div>'),
    (esAsistente ? '' : seccionEliminados(opciones)),
    '<div class="admin-sec" data-sec="realizados">' + seccionRealizados(opciones) + '</div>',
...
function correoNotificacion(reg) {
  const cuerpo = [
    ...
    filaCorreo('Adelanto (urgencia)', reg.adelanto === 'solo'
      ? 'Sí — pago de adelanto suelto (entrega en máx. 3 días)'
      : (reg.adelanto === 'incluido' ? 'Sí — incluye adelanto (entrega en máx. 3 días)' : '')),
...
/* ---------- Correo AL ADMINISTRADOR: aviso de entrega próxima ---------- */
// Se envía cuando un trabajo agendado (aún no realizado) está cerca de su
// fecha límite de entrega: 3 días si tiene adelanto (urgencia), 6 semanas si no.
function correoAvisoEntrega(reg, info) {
  const d = info || {};
  const conAdelanto = !!d.conAdelanto;
  const plazoTexto = conAdelanto ? '3 días (tiene adelanto de urgencia)' : '6 semanas';
  const restanteTexto = d.diasRestantes <= 0
    ? '<strong style="color:#f0a3c3;">¡El plazo ya venció!</strong>'
    : 'Queda' + (d.diasRestantes === 1 ? '' : 'n') + ' <strong style="color:#f0a3c3;">' +
      d.diasRestantes + ' día' + (d.diasRestantes === 1 ? '' : 's') + '</strong> para el límite de entrega.';
  const cuerpo = [
    '<p style="color:#f0a3c3;font-size:15px;font-weight:700;margin:0 0 16px;">' + escaparHtml(reg.producto || 'Servicio') + '</p>',
    '<table style="width:100%;border-collapse:collapse;">',
    filaCorreo('Cliente', reg.cliente_nombre),
    ...
    filaCorreo('Persona a trabajar', reg.objetivo_nombre),
    filaCorreo('Referencia', reg.ref),
    filaCorreo('Adelanto (urgencia)', conAdelanto ? 'Sí — entrega en máx. 3 días' : 'No'),
    filaCorreo('Agendado el', formatearFecha(d.desde)),
    filaCorreo('Fecha límite de entrega', formatearFecha(d.limite)),
    '</table>',
    notaCorreo('Este trabajo sigue <strong style="color:#f0a3c3;">pendiente</strong> y su plazo de entrega es de ' +
      plazoTexto + '. ' + restanteTexto +
      ' Cuando lo realices, márcalo como hecho en el panel de administración para dejar de recibir este aviso.'),
  ].join('');
  return envolturaCorreo({
    icono: conAdelanto ? '⚡' : '✦',
    titulo: conAdelanto ? 'Entrega urgente próxima' : 'Entrega próxima',
    preheader: (reg.producto || 'Servicio') + ' · ' + (reg.cliente_nombre || 'Cliente'),
    cuerpo: cuerpo,
    pie: 'Recordatorio automático · Fresatanika',
  });
}
...
module.exports = {
  ...
  correoAvisoEntrega: correoAvisoEntrega,
